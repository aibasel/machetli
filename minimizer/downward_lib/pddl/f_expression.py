...
class PrimitiveNumericExpression(FunctionalExpression):
    ...
    def dump_pddl(self, output, indent=""):
        output.write("(%s %s)" % (self.symbol, " ".join(map(str, self.args))))
...
class FunctionAssignment:
    ...
    def dump_pddl(self, output, indent=""):
        output.write("%s(%s " % (indent, self._pddl_keyword()))
        self.fluent.dump_pddl(output)
        output.write(" ")
        self.expression.dump_pddl(output)
        output.write(")\n")
